...
# Make script
cat > slim_constant_second_contraction_${Na}.slim << EOM
...
// track statistics for P1 every 50 generation
1:10000 late() {
if (sim.generation % 50 == 0) {
        if(p1.individuals.size() < 2){
                stats_P1 = c("NA,NA,NA,NA,NA,NA,NA,NA"); //cant get stats from just one individual
        }
        if(p1.individuals.size() < sampleSize & p1.individuals.size() > 1){     // case when p1 size is less than sample size but greater than 1
                stats_P1 = getStats(p1, p1.individuals.size());
        }
        if(p1.individuals.size() >= sampleSize){ //case when p1 size is greater than or equal to sample size
                stats_P1 = getStats(p1, sampleSize);
        }

        cat(sim.generation + "," + stats_P1 + "\n");
